"""
Shared evaluation metrics -- kept separate from baseline_model.py so the
same functions plug straight into step 4 (MLflow logging) and step 5
(Optuna objective function) without duplicating anything.

Three metrics, each answering a different question:
  - WAPE:  "on average, how far off were we, in units of actual volume?"
  - MASE:  "are we better than a naive seasonal guess?" (comparable across
           SKUs with very different volume, unlike WAPE or raw MAE)
  - bias%: "are we systematically over- or under-forecasting?" -- this is
           the one your example ("accuracy was great but we under-produced")
           is actually about; WAPE/MASE alone can look fine while this is
           badly skewed in one direction.
"""
